...
\title{Reinforcement learning accelerates expertise in skilled alpine ski racers}
...
Developing expertise demands substantial amounts of high-quality training \cite{ericsson_role_1993, ericsson_expert_1994, vaeyens_talent_2009, williams_expertise_2008}. Unlike training novices to reach acceptable levels on a task, the main challenge for skilled performers is finding ways to improve beyond current levels of performance\cite{ericsson_development_2003, ericsson_scientific_1998, gray_plateaus_2017, williams_expertise_2008, du_relationship_2022}. One route to achieve this goal is to perfect an already selected strategy. This is a slow improvement process that operates through honing the components underlying the automated solution\cite{du_relationship_2022}. A second route is to find new and superior ways to solve the task, which can sometimes lead to greater improvement leaps than simply repeating what is already automatized \cite{gray_plateaus_2017, du_relationship_2022}. Finding such creative and innovative solutions is a hallmark of expertise development; however, we know little about which training strategies are most effective at developing these strategies. If superior methods to those currently used in standard practice exist, they could prove invaluable for training both present and future generations of learners \cite{williams_expertise_2008}.
...
We found that the reinforcement learning group improved more during acquisition and performed better in retention compared to the supervised (free choice) learning group. Alongside this improvement, both groups improved their choice of strategy to ski fast in the slalom course. However, we did not observe any statistically significant differences in choices between groups, either for the theoretically best strategy or individual skiers' estimated best strategy. 

We observed an improvement in strategy choices for both reinforcement learning and supervised (free choice) learning groups, but we did not observe any statistically significant differences in choices between groups, either for the theoretically best strategy or individual skiers' estimated best strategy. We also found that both groups were sensitive to performance feedback, exhibiting win-stay, lose-shift behavior in strategy choices, but the descriptively greater sensitivity to feedback in the reinforcement learning group was not statistically significant. Our results suggest that reinforcement learning improves skill learning compared to the standard supervised learning approach


[Our results suggest that reinforcement learning yields comparable outcomes to supervised learning, and that any advantages of reinforcement learning over supervised learning are more likely rooted in effects on skill execution rather than action selection. ] - this needs to be changed
...
An unexpected discovery was that, compared with reinforcement learning, supervised (targeted skill) learning did not learn to differentiate between 'extend' and 'rock skis forward' in terms of their effects. This observation suggests that supervised learning may miss potential learning by picking only the theoretical best strategy and not exploring other alternative strategies. Over time, such exploration could prove crucial in developing innovative strategies, as athletes cultivate a deeper comprehension of the relationship between their actions and performance outcomes \cite{ericsson_scientific_1998} In the future, it is imperative to explore how different learning strategies influence both performance and innovation within this domain.
